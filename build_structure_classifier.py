...
No DFT-derived properties used as features: no formation energy, volume,
density, band gap, magnetization, or total energy.

Step 1: Query training data (same 28 binary systems as build_dataset.py)
Step 2: Generate structure-only features via matminer + pymatgen
Step 3: Train GBC and RF; report CV, test metrics, feature importances
Step 4: Validate on Fe-Sb and Ni-Sb holdouts; compare to DFT-feature model
Step 5: Score Co-Bi candidates from cobi_test_combinations_100.csv
        (file is auto-generated if absent)
"""
...
SUMMARY_FIELDS  = [
    "material_id","formula_pretty","chemsys","symmetry","nsites","energy_above_hull",
]
...
DFT_COLS        = {
    ...
    "volume","density","band_gap","total_magnetization","magnetic_ordering",
    "volume_per_atom","energy_per_atom","total_energy",
}

# ── Co-Bi candidate prototypes ────────────────────────────────────────────────
# Each row: (formula, sg_number, crystal_system, nsites, sg_symbol, prototype_name)
# Represents a hypothetical structure to be screened before DFT.
...
_COBI_PROTOTYPES = [
    ...
    # ---- larger supercells of promising prototypes ----
    ...
    # ---- additional high-symmetry variants ----
...
def engineer_features(df: pd.DataFrame) -> tuple[pd.DataFrame, list[str]]:
    """
    Build structure-only feature matrix. Returns (df_with_features, feat_cols).
    All DFT-derived fields are excluded from feat_cols.
    """
    from pymatgen.core import Composition
    ...
    for col in CS_COLS:
        if col not in cs_dummies.columns:
            cs_dummies[col] = 0
    extra = [c for c in cs_dummies.columns if c not in CS_COLS]
    ...
    ep_data = [safe_featurize(ep, c) if c else [np.nan]*len(ep_names)
               for c in df["_comp"]]
    st_data = [safe_featurize(st, c) if c else [np.nan]*len(st_names)
               for c in df["_comp"]]
    vo_data = [safe_featurize(vo, c) if c else [np.nan]*len(vo_names)
               for c in df["_comp"]]
    ...
    skip = DFT_COLS | {"material_id","formula_pretty","chemsys","crystal_system"}
...
def align_to_training(df: pd.DataFrame, feat_cols: list[str]) -> np.ndarray:
    """
    Apply the same structural + compositional preprocessing, then align columns
    to the training feature list. For use on holdout/candidate data.
    """
...
def generate_candidates_csv(path: Path):
    ...
    for i, (formula, sg_num, cs, nsites, sg_sym, proto) in enumerate(_COBI_PROTOTYPES, 1):
        co_frac = _first_el_fraction(formula) if "Co" in formula else 0.0
        # co is alphabetically before bi? No: Bi < Co alphabetically.
        # composition_A_fraction = fraction of alphabetically first element = Bi fraction
        ...
        rows.append({
            ...
            "crystal_system":  cs,
            "n_atoms":         nsites,
            "co_fraction":     1.0 - bi_frac,   # Bi is alphabetically first, so A=Bi
            "bi_fraction":     bi_frac,
...
def _build_candidate_features(df_cand: pd.DataFrame, feat_cols: list[str]) -> np.ndarray:
    ...
    # rename columns to match engineering pipeline
    renames = {
    ...
    # impute any NaNs using column medians (crude but sufficient for inference)
...
def print_comparison_table(structure_holdouts: list[dict]):
    ...
    print("  Structure-only model uses 0 DFT outputs -- valid pre-screening.")
...
def main():
    ...
    # ── Step 1: query training data ────────────────────────────────────────
    print("=" * 55)
    print("Step 1: Query training data (structure fields only)")
    print("=" * 55)
    train_raw_path = out_dir / "training_data_structure_raw.csv"
    if train_raw_path.exists():
        print(f"  Loading cached raw data from {train_raw_path}")
    ...
    # ── Step 2: feature engineering ────────────────────────────────────────
    print(f"\n{'='*55}")
    print("Step 2: Structure-only feature engineering")
    print(f"{'='*55}")
    ...
    df_cand   = load_or_generate_candidates(cand_path)

    print("  Building candidate feature matrix...")
    ...
    print("(Recommended shortlist for downstream DFT calculation)")
    print(f"{'='*70}")
